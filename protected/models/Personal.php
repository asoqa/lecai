...
class Personal extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('issue, count, status', 'numerical', 'integerOnly'=>true),
			array('prize', 'numerical'),
			array('user_id, selection', 'length', 'max'=>45),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, user_id, issue, selection, count, status, prize', 'safe', 'on'=>'search'),
		);
	}
